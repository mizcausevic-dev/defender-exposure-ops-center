...
function layout(title: string, active: string, body: string) {
  const nav = [
    { href: "/", label: "Overview" },
    { href: "/exposure-lane", label: "Exposure Lane" },
    { href: "/control-gaps", label: "Control Gaps" },
...
<html lang="en">
  ...
  <body>
    ...
    <div class="wrap">
      <div class="topbar">
        <div class="left">Kinetic Gain · Defender Exposure Ops Center</div>
        <div class="right">
          <div>synthetic defender controls · remediation packets</div>
          <div>microsoft defender · exposure ops · identity + device posture</div>
        </div>
      </div>
      <div class="herorow">
        <section class="hero">
          <div class="chiprow">
            ...
            <span class="meta-chip">Microsoft Defender / exposure management proof</span>
            <span class="meta-chip">Synthetic control + recommendation exports</span>
          </div>
          <h1>Microsoft Defender exposure operations that stay operator-readable.</h1>
          <p>This control plane turns Defender exposure data into one buyer-readable surface: attack-path posture, privileged identity risk, device coverage gaps, stale remediation, and the response packets needed before change windows, audits, or tenant trust drift.</p>
          <div class="navrow">
        ...
        <aside class="side">
          <div class="bluf">
            ...
            <p><strong>Enterprise Microsoft Defender exposure operations for identity, endpoint, and collaboration teams.</strong><br />Audit-safe visibility into attack paths, device risk, and remediation posture without exposing tenant credentials or raw admin surfaces.</p>
          </div>
          <div class="corr">
            <div class="lbl">Proof Layer</div>
            <p><strong>Offline analyzer + CLI + dashboard surface.</strong><br />This repo includes a reusable analyzer that reads Defender exposure snapshots and turns them into control, gap, and remediation packets.</p>
          </div>
          <div class="corr">
            <div class="lbl">Why it matters</div>
            <p>Recruiters looking for <strong>Azure / Microsoft 365 / Entra / Intune</strong> should see real operator work: attack paths, privileged access, device risk, and remediation sequencing.</p>
          </div>
        </aside>
      </div>
      ${body}
      <div class="footer">
        <div>defender-exposure-ops-center · synthetic sample data only</div>
        <div>routes: / · /exposure-lane · /control-gaps · /remediation-posture · /verification · /docs</div>
      </div>
    </div>
  </body>
</html>`;
...
export function renderOverview() {
  ...
  return layout(
    ...
    `<section class="section">
        <div class="sh"><h2>Operator Snapshot</h2><div class="note">attack paths · device risk · remediation posture</div></div>
        <div class="kpis">
          <div class="kpi cyan"><div class="v">${metrics.controls}</div><div class="lbl">controls</div><div class="h">Synthetic Defender control records across global and regional scopes.</div></div>
          <div class="kpi green"><div class="v">${metrics.healthyControls}</div><div class="lbl">healthy controls</div><div class="h">Controls currently carrying healthy exposure-management coverage.</div></div>
          <div class="kpi plum"><div class="v">${metrics.recommendations}</div><div class="lbl">recommendations</div><div class="h">Exposure recommendations across identity, device, collaboration, and vulnerability posture.</div></div>
          <div class="kpi red"><div class="v">${metrics.highSeverityRecommendations}</div><div class="lbl">high recommendations</div><div class="h">High-severity Defender recommendations needing the fastest operator path.</div></div>
          <div class="kpi amber"><div class="v">${metrics.attackPathSignals}</div><div class="lbl">attack path signals</div><div class="h">Active chained-risk paths still visible in the current board.</div></div>
          <div class="kpi red"><div class="v">${metrics.staleRecommendations}</div><div class="lbl">stale active recommendations</div><div class="h">Recommendations that have remained open longer than the remediation SLA.</div></div>
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Why operators care</h2><div class="note">exposure ops · response evidence · recruiter signal</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">containment first</div><div class="src-tit">Route the exposure before trust slips</div><p>${metrics.recommendation}</p></div>
          <div class="src"><div class="src-name">operator evidence</div><div class="src-tit">Turn Defender exports into control-plane proof</div><p>Every lane stays tied to owner, remediation focus, control health, and the next concrete operator move.</p></div>
          <div class="src"><div class="src-name">recruiter signal</div><div class="src-tit">Show real Microsoft security depth</div><p>This is real Defender exposure operations and remediation proof, not generic cloud-security copy.</p></div>
        </div>
      </section>`
  );
}

export function renderExposureLane() {
  return layout(
    "Defender Exposure Ops Center — Exposure Lane",
    "/exposure-lane",
    `<section class="section">
        <div class="sh"><h2>Exposure Lane</h2><div class="note">owner · focus · next action</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Lane</th>
...
export function renderControlGaps() {
  return layout(
    "Defender Exposure Ops Center — Control Gaps",
    "/control-gaps",
    `<section class="section">
        <div class="sh"><h2>Control Gaps</h2><div class="note">severity · owner · principal</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Gap</th>
...
export function renderVerification() {
  return layout(
    "Defender Exposure Ops Center — Verification",
    "/verification",
    `<section class="section">
        ...
        <div class="stack">
          ${verification()
            .map(
              (item, index) => `<div class="src"><div class="src-name">verification ${index + 1}</div><div class="src-tit">${item}</div><p>This surface is built to stay honest about offline exports, synthetic sample data, and real Microsoft Defender exposure posture.</p></div>`
            )
            .join("")}
        </div>
      </section>`
...
export function renderDocs() {
  return layout(
    "Defender Exposure Ops Center — Docs",
    "/docs",
    `<section class="section">
        <div class="sh"><h2>Docs</h2><div class="note">routes · cli · api</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">routes</div><div class="src-tit">Public control surface</div><p><code>/</code>, <code>/exposure-lane</code>, <code>/control-gaps</code>, <code>/remediation-posture</code>, <code>/verification</code>, <code>/docs</code></p></div>
          <div class="src"><div class="src-name">api</div><div class="src-tit">Structured payloads</div><p><code>/api/dashboard/summary</code>, <code>/api/exposure-lane</code>, <code>/api/control-gaps</code>, <code>/api/remediation-posture</code>, <code>/api/verification</code>, <code>/api/sample</code></p></div>
          <div class="src"><div class="src-name">cli</div><div class="src-tit">Offline Defender analysis</div><p><code>npx defender-exposure-ops fixtures/defender-exposure-clean.json --format summary</code> renders the same remediation posture the dashboard exposes.</p></div>
        </div>
      </section>`
  );
}
